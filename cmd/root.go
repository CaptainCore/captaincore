...
func resolveCommand(c *cobra.Command, args []string) {
	...
	if target_count > 0 && c.CommandPath() != "captaincore monitor" && c.CommandPath() != "captaincore bulk" && c.CommandPath() != "captaincore plugin-zip" && c.CommandPath() != "captaincore backup get" && c.CommandPath() != "captaincore backup download" {
