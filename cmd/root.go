...
func init() {
	cobra.OnInitialize(initConfig, ensureAssets)
...
func resolveCommand(c *cobra.Command, args []string) {
	...
	if target_count > 0 && c.CommandPath() != "captaincore monitor" &&
		...
		c.CommandPath() != "captaincore update-log generate" && c.CommandPath() != "captaincore update-log list-generate" && c.CommandPath() != "captaincore update-log get" && c.CommandPath() != "captaincore capture generate" && c.CommandPath() != "captaincore capture scan" && c.CommandPath() != "captaincore capture check" {
