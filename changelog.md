...
- **Backups of WP Freighter installs store each part once.** `vault create` (direct-mode backups) used to snapshot the whole web root and the whole database for the host and again for every tenant, so a host with twelve tenants stored its install thirteen times a night. It now asks the site what it is: a tenant's snapshot holds only its own `stacked_<id>_` tables and `content/<id>` (just the database in shared files mode), and the host's leaves out every registered tenant's tables and `content/<id>`. Paths stay relative to the web root, so browsing and restoring work as before, and restoring a host's database no longer overwrites its tenants' tables. Snapshots are tagged `freighter-host` or `freighter-tenant:<id>`. The window where the dump sits in the web root is now locked per install (`private/.vault-root.lock`), since tenants of one host backing up at once overwrote each other's `database-backup.sql`. Local-mode backups (`backup_settings.mode = local`) are not changed yet.
