...
- **Network shape in `sync-data`.** `fetch-site-data` now reports multisite subsites and WP Freighter tenants as two JSON keys stored in environment details. `network_sites` lists each subsite (id, name, URL, main flag, registered and last-updated dates, public/archived/spam/deleted) with the network mode and exact count. `freighter` gives the site's role (`host` or `tenant`), the tenant id, the host's main URL, files mode, domain-mapping state and, on a host, its tenant list (id, name, domain, URL when mapped, created). Both read tables directly with plugin code off, lists are capped at 500, only the files mode and domain-mapping flag are taken from the Freighter settings (they also hold the license key), and a site that is neither sends `null` so a stale value is cleared on the Manager. A probe that fails sends neither key. Measured at 0.3s on a 19-tenant Freighter host and 1s on a 228-subsite network.
- **Administrators per subsite and per tenant.** `network_sites` now lists up to ten administrators (login and email) on each subsite plus the network's super admins, and a Freighter host lists each tenant's administrators from that tenant's own user tables and whether its CaptainCore helper can mint login links (a stub helper that only redeems links does not count). The Manager signs into a subsite or tenant with these, since a main-site admin may not be a member there.
- **`captaincore ssh <site> --tenant=<id>`** runs a command, script or recipe inside one WP Freighter tenant of the site by exporting `STACKED_SITE_ID` first, so tenants that are not CaptainCore sites can be reached through their host (single site only; the id must be numeric). The Manager uses it to install the helper on a tenant before signing in.
- **No Fathom tracker for a WP Freighter tenant without its own domain.** `site stats-generate` skips an environment whose `freighter` detail says it is a tenant answering on its host's URL, since a tracker there would count the host's traffic. `Environment.IsUnmappedTenant()` makes the same check available elsewhere.
- **Backups of WP Freighter installs store each part once.** `vault create` (direct-mode backups) used to snapshot the whole web root and the whole database for the host and again for every tenant, so a host with twelve tenants stored its install thirteen times a night. It now asks the site what it is: a tenant's snapshot holds only its own `stacked_<id>_` tables and `content/<id>` (just the database in shared files mode), and the host's leaves out every registered tenant's tables and `content/<id>`. Paths stay relative to the web root, so browsing and restoring work as before, and restoring a host's database no longer overwrites its tenants' tables. Snapshots are tagged `freighter-host` or `freighter-tenant:<id>`. A tenant's dump is named `database-backup-<tenant id>.sql` (in `private/` and in its snapshots), because tenants of one host back up at the same time and all staged `database-backup.sql` in the same web root, overwriting each other; the host's snapshot leaves any staged tenant dump out. `backup verify` expects the tenant's name, and the quicksave database restore reads either name, so tenant snapshots from before the change still restore. Local-mode backups (`backup_settings.mode = local`) are not changed yet.
...
- **Lessons from a SMILODON incident audit.** The toolkit's shapes are rules now: image files made of printable text (`PNG`/`GIF` letters then base64, its payload carrier), includes whose path is built from string fragments, the two-alphabet `strtr` decoder, the self-hide variants that unset the plugin row before adding the filter or through `pre_current_active_plugins`, its constants, cookie names and C2 hosts, and a login hook that sends the password away; the twelve files of the build are hash indicators and corpus samples. The database scan exports its session markers, options named `wp_<md5 of an IP>` with a timestamp value, which survived five file-only cleanups on that site, and rates them high; its `__`-prefixed config rows are not used as a tell because Spectra keeps `__uagb_*` options everywhere. Security plugins' own signature options (VaultPress's `_vp_signatures` and friends) are left out of the export, UpdraftPlus's `backupdb` option name is no longer a known-injection name, and Real Media Library's stored functions are recognised. Rule findings on database rows keep their severity only for the families that mean the same in data as in a file (spam links, skimmers, injected script, C2 indicators, known hashes); PHP-execution shapes in an option or a comment are fuzzing residue unless something evaluates them, so they are capped at medium and recorded rather than emailed (a Divi email option full of saved scanner payloads was the first case). WC Blacklist Manager's counter triggers are recognised. A "new administrator" now also has to be registered within the last 30 days: the fleet run flagged accounts from 2016 on sites whose stored user list was partial (WooCommerce stores hold thousands of customers, and the list is cut somewhere on the way). `sync-data` now alerts when a security plugin (WP 2FA, Wordfence, Solid Security and others) was active at the previous sync and is inactive or gone, since the backdoor switched WP 2FA off by writing `active_plugins` directly with no deactivation event anywhere.
- **Every finding is reported; the Manager decides what emails.** The CLI no longer applies the high floor before posting: findings of every severity go to the Manager with their severity and family, the hidden-plugin, media, database and security-plugin checks included. The Manager (from its matching release) emails the proven tier-1 classes at once and stores the rest for the daily review. Deploy the Manager first: an older Manager emails everything it receives.
...
- **Rules can veto on a pattern, and the first tier-1 review's false positives are pinned.** A rule may now carry `exclude_patterns`: RE2 patterns any one of which, matched anywhere in the file, keeps the rule from firing. It states a vendor shape once instead of growing an `exclude_paths` list, and a theme registration form that hands out `get_option('default_role')` is the first use. From the alert review of the same night: the split-string rule needs the base64 fragment to lead into `_decode` (a split `_encode` in a license helper is not a sink); phpDocumentor's own tests include a `data:` URI on purpose; the NightJar rule matches the kit's class names, not the word in a security page's prose; and the SourceCop encoder (a dirname walk to `scopbin/`, then a ciphered blob, as HappyPlugins ships) is listed once at low as `sourcecop-encoded-file` rather than twenty high hits from an amwscan signature that describes the encoder itself. Each shape and its malicious twin are fixtures.
- **Capture checks know a site's other environments.** A staging copy that still loads its stylesheet from the production domain was reported as an unknown external host; every environment's home URL of the same site now counts as same-origin. Weglot's CDN joins the known-safe list.
